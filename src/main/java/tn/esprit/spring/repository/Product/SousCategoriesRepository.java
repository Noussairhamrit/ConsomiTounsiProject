...
public interface SousCategoriesRepository extends JpaRepository<SousCategories,Long> {
	...
	@Query(value = "SELECT * FROM sous_cat WHERE sous_cat_nom=?1", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM sous_cat WHERE categorie_cat_id=?1", nativeQuery = true)
	public List<SousCategories> findByIdCategories(Long id);
